...
**환경변수:** `process.env.*` 직접 참조 금지 → `import.meta.env.*` 사용 · 반드시 `constants/config.ts`에서 래핑 후 사용 · 미설정 시 앱 시작 전 명시적 에러를 던져 조기 실패 유도
...
**레이어:** Component → Hook → Store / Constants / Types 단방향 · 컴포넌트는 UI만(비즈니스 로직 금지) · `utils/keyState`는 `useFrame` 안에서만 직접 참조 허용
