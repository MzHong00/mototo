...
**Export:** 컴포넌트 named · **배럴 파일(`index.ts`) 금지** — 트리 쉐이킹 방해·내부 경로 은폐로 디버깅 불편. 항상 실제 파일 경로로 직접 import(`@/constants/skill/skill`, `@/utils/controls` 등)
...
**참조 검토:** [중요]코드 생성, 수정, 삭제 시 AGENTS.md에서 적용한 규칙들 항목을 모두 출력해서 사용자에게 규칙 문서를 잘 지켰는지 알림
