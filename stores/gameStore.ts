...
export type JobClass = "warrior" | "archer" | "mage";
...
interface GameState {
  ...
  addFX: (type: SkillFX["type"], pos: [number, number, number]) => void;
...
// 직업별 초기 스탯 + 스킬
const CLASS_CONFIG: Record<JobClass, {
  hp: number; mp: number; atk: number; def: number;
  skills: SkillState[];
}> = {
  warrior: {
    hp: 150, mp: 30, atk: 20, def: 5,
    skills: [
      { id: "slash",  key: "Q", label: "베기",    mpCost: 0,  cooldown: 0.5, lastUsed: 0 },
      { id: "shield", key: "W", label: "방패막기", mpCost: 8,  cooldown: 8,  lastUsed: 0 },
      { id: "heal",   key: "E", label: "투지",     mpCost: 10, cooldown: 10, lastUsed: 0 },
      { id: "blast",  key: "R", label: "회오리",   mpCost: 20, cooldown: 18, lastUsed: 0 },
    ],
  },
  archer: {
    hp: 100, mp: 60, atk: 22, def: 2,
    skills: [
      { id: "slash",  key: "Q", label: "연사",     mpCost: 0,  cooldown: 0.4, lastUsed: 0 },
      { id: "shield", key: "W", label: "회피",     mpCost: 12, cooldown: 10, lastUsed: 0 },
      { id: "heal",   key: "E", label: "치료약",   mpCost: 15, cooldown: 12, lastUsed: 0 },
      { id: "blast",  key: "R", label: "폭발화살", mpCost: 25, cooldown: 20, lastUsed: 0 },
    ],
  },
  mage: {
    hp: 80, mp: 120, atk: 30, def: 0,
    skills: [
      { id: "slash",  key: "Q", label: "화염볼",   mpCost: 5,  cooldown: 0.6, lastUsed: 0 },
      { id: "shield", key: "W", label: "냉기장벽", mpCost: 15, cooldown: 12, lastUsed: 0 },
      { id: "heal",   key: "E", label: "마나흡수",  mpCost: 0,  cooldown: 15, lastUsed: 0 },
      { id: "blast",  key: "R", label: "메테오",   mpCost: 40, cooldown: 25, lastUsed: 0 },
    ],
  },
};
...
export const useGameStore = create<GameState>((set, get) => ({
  character: {
    name: "플레이어", jobClass: null, level: 1,
    hp: 100, maxHp: 100, mp: 50, maxMp: 50,
    exp: 0, expToNext: EXP_PER_LEVEL(1),
    baseAtk: 15, baseDef: 0,
  },
  skills: CLASS_CONFIG.warrior.skills,
  inventory: [], equipped: { weapon: null, armor: null, ring: null },
  gold: 0,
  isShielded: false, shieldEndTime: 0,
  isDead: false, levelUpPending: false, shopOpen: false, fxList: [],
  ...
  selectClass: (cls) => {
    ...
    set((state) => ({
      character: {
        ...
        mp: cfg.mp, maxMp: cfg.mp,
        baseAtk: cfg.atk, baseDef: cfg.def,
      },
      skills: cfg.skills,
    }));
  },
    ...
    set((state) => {
      ...
      if (newExp >= needed) {
        ...
        return {
          levelUpPending: true,
          character: {
            ...character, level: lv, exp: newExp - needed,
            expToNext: EXP_PER_LEVEL(lv),
            maxHp: mhp, hp: mhp, maxMp: mmp, mp: mmp,
  ...
  healHp: (n) => set((s) => ({ character: { ...s.character, hp: Math.min(s.character.maxHp, s.character.hp + n) } })),
  healMp: (n) => set((s) => ({ character: { ...s.character, mp: Math.min(s.character.maxMp, s.character.mp + n) } })),
  ...
  useSkill: (id) => {
    ...
    set((state) => ({
      ...
      skills: state.skills.map((s) => s.id === id ? { ...s, lastUsed: Date.now() } : s),
    }));
    return true;
  },

  activateShield: () => set({ isShielded: true, shieldEndTime: Date.now() + 4000 }),
  tickShield: () => {
  ...
  addItem: (item) => set((s) => ({ inventory: s.inventory.length < 16 ? [...s.inventory, item] : s.inventory })),
  ...
  equipItem: (item) =>
    set((state) => {
      ...
      return {
        ...
        inventory: state.inventory.filter((i) => i.uid !== item.uid).concat(prev ? [prev] : []),
        character: {
          ...state.character,
          maxHp: state.character.maxHp + hpDelta,
          hp: Math.min(state.character.hp + hpDelta, state.character.maxHp + hpDelta),
        },
      };
    }),
  ...
  addFX: (type, pos) => set((s) => ({ fxList: [...s.fxList, { fxId: fxCounter++, type, pos, startTime: Date.now() }] })),
