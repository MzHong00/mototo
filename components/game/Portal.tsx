...
export function Portal({ zone, onEnter }: PortalProps) {
  ...
  useFrame(({ clock }) => {
    ...
    if (outerRef.current) { outerRef.current.rotation.y = t * 1.2; outerRef.current.rotation.z = t * 0.5; }
    if (innerRef.current) { innerRef.current.rotation.y = -t * 0.8; innerRef.current.scale.setScalar(1 + Math.sin(t * 2) * 0.05); }
