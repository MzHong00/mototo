...
export function FireballFX({ fx }: { fx: SkillFX }) {
  ...
  useFrame(() => {
    ...
    const dist = age * 9;
    groupRef.current.position.set(fx.pos[0] + dir.x * dist, fx.pos[1] + 1.0, fx.pos[2] + dir.z * dist);
  ...
  return (
    <group ref={groupRef} position={[fx.pos[0], fx.pos[1] + 1.0, fx.pos[2]]}>
      <mesh>
        ...
        <meshStandardMaterial ref={coreMat} color="#FF4400" emissive="#FF2200" emissiveIntensity={1.5} transparent />
      </mesh>
      <mesh>
        <sphereGeometry args={[0.45, 8, 6]} />
        <meshBasicMaterial ref={glowMat} color="#FF8800" transparent opacity={0.35} side={THREE.BackSide} />
      </mesh>
    </group>
  );
}
